...
let
	...
	convertBin = "${pkgs.imagemagick}/bin/convert";
	python = "${pkgs.python3}/bin/python3";
	fishBin = "${pkgs.fish}/bin/fish";
	sensorsBin = "${pkgs.lm_sensors}/bin/sensors";
	awkBin = "${pkgs.gawk}/bin/awk";
	ipBin = "${pkgs.iproute2}/bin/ip";
	bcBin = "${pkgs.bc}/bin/bc";
	wofiBin = "${pkgs.wofi}/bin/wofi";
	...
	cputemp-script = pkgs.writeShellScriptBin "waybar-cputemp" ''
		temp=$(${sensorsBin} coretemp-isa-0000 2>/dev/null \
			| ${awkBin} '/Package id 0:/ {gsub(/[^0-9.]/, "", $4); print int($4)}')
	...
	netspeed-script = pkgs.writeShellScriptBin "waybar-netspeed" ''
		iface=$(${ipBin} route | ${awkBin} '/default/ {print $5; exit}')
		[ -z "$iface" ] && iface="enp4s0"

		rx1=$(${awkBin} "/$iface:/ {print \$2}" /proc/net/dev)
		tx1=$(${awkBin} "/$iface:/ {print \$10}" /proc/net/dev)
		sleep 1
		rx2=$(${awkBin} "/$iface:/ {print \$2}" /proc/net/dev)
		tx2=$(${awkBin} "/$iface:/ {print \$10}" /proc/net/dev)
		...
		fmt_speed() {
			local kb=$1
			if [ "$kb" -ge 1024 ]; then
				printf "%.1f MB/s" "$(echo "scale=1; $kb/1024" | ${bcBin})"
			else
				printf "%d KB/s" "$kb"
			fi
		}
...
" 2>/dev/null || true
	...
	wallpaper-picker = pkgs.writeShellScriptBin "wallpaper-picker" ''
		...
		chosen=$(ls "$BG_DIR" | ${wofiBin} --dmenu --prompt "  Wallpaper" --width 380 --height 300 --no-actions)
		[ -z "$chosen" ] && exit 0
		...
		read -r R G B <<< "$(${python} ${extractAccentPy} "$path" "${convertBin}")"
